    ...
    def dist_forward_collect(self, data_loader, length=None, gather_data=False, mode = "Train"):
        ...
        with progress:
            ...
            for idx, (batch_x, batch_y) in enumerate(data_loader):
                ...
                loss = self.cal_loss(pred_y, batch_y).cpu().numpy()

                if gather_data:  # return raw datas
                    results.append(dict(zip(['inputs', 'preds', 'labels', 'loss'],
                                            [batch_x.cpu().numpy(), pred.cpu().numpy(), batch_y.cpu().numpy(), np.array([loss])])))
                else:  # return metrics
                    eval_res, _ = metric(pred.cpu().numpy(), batch_y.cpu().numpy(),
